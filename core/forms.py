...
from django import forms
from .models import ScholarshipPosting, Scholarship, AcademicYear, Student


class ScholarshipPostingForm(forms.ModelForm):
    student_no = forms.IntegerField(
        label="Student No",
        widget=forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Enter student number'})
    )

    class Meta:
        model = ScholarshipPosting
        fields = ['academic_year', 'student_no', 'scholarship', 'amount']
        widgets = {
            'academic_year': forms.Select(attrs={'class': 'form-control'}),
    ...
    def clean_student_no(self):
        ...
        try:
            ...
            raise forms.ValidationError("No student found with this Student No.")
        return student
from django import forms
